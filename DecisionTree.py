...
def entropy(rows):
    ...
    for lbl in counts:
        prob_of_lbl = counts[lbl] / float(len(rows))
        uncertainity -= prob_of_lbl*(math.log2(prob_of_lbl))
    return uncertainity
...
class Leaf:
    ...
    def __init__(self, rows, id, depth):
        ...
        for key in self.predictions:
            self.predictedLabel =key  
        #self.predictedLabel = label_counts(row)
        self.id=id
        self.depth=depth
...
## TODO: Step 4
def print_tree(node, spacing="",depth=0,id=0):
    ...
    print (spacing + "Question : "+str(node.question)+"Depth : "+str(node.depth)+"id : "+str(node.id))

    # Call this function recursively on the true branch
    print (spacing + '--> True:')
    print_tree(node.true_branch, spacing + "  ",node.depth+1, 2*(node.id)+2)

    # Call this function recursively on the false branch
    print (spacing + '--> False:')
    print_tree(node.false_branch, spacing + "  ",node.depth+1, 2*(node.id)+1)
...
def getLeafNodes(node, leafNodes =[]):
    ...
    if isinstance(node, Leaf): # if it is the leaf node then the node will be added to the leafNodes list
        leafNodes.append(node)
        return 
    #Recursion to check nodes of the true branch
    getLeafNodes(node.true_branch,leafNodes)
        
    #Recursion to check nodes of the false branch
...
def getInnerNodes(node, innerNodes =[]):
    ...
    if isinstance(node, Leaf):# if it is the nonleaf node then the node will be added to the innerNodes list
       return 
...
def computeAccuracy(rows, node):
    ...
    if len(rows) == 0:
        return 0
    for row in rows:
        predictedLbl = classify(row,node)
        if predictedLbl == row[-1]:
            accuracy +=1

    return round(accuracy/len(rows),2)
